  ...
  return (
    <div className={`mx-4 my-3 p-4 rounded-xl border ${bg}`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-gray-500">本轮形成性诊断</span>
        <span className={`text-2xl font-bold ${color}`}>{score}</span>
      </div>
      <p className="text-sm text-gray-700 mb-3">{msg.feedback}</p>
      {msg.keyPointsHit && msg.keyPointsHit.length > 0 && (
        <div className="mb-2">
          <span className="text-xs text-green-600 font-medium">已掌握证据：</span>
      ...
      {msg.keyPointsMissed && msg.keyPointsMissed.length > 0 && (
        <div>
          <span className="text-xs text-red-600 font-medium">待补知识或步骤：</span>
          <div className="flex flex-wrap gap-1 mt-1">

...
export function ChatWindow({ wsRef }: ChatWindowProps) {
  ...
  const handleStartTraining = () => {
    ...
    const studentProfile = studentProfileFile
      ...
      : studentProfileText || '暂无补充历史，请从本轮作答开始诊断。'
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-gray-400">
            <p className="text-lg mb-2">Student-Coach</p>
            <p className="text-sm">围绕学科、章节和知识点开展自适应练习、追问与掌握度诊断</p>
          </div>
        )}
        {messages.map((msg) => (
      ...
      {showTrainingSetup && (
        <div className="px-4 py-4 border-t bg-gray-50">
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">学科、教材、章节与学习目标</label>
              <FileUpload
                label="上传教材章节、课程目标或学习材料"
              ...
              <textarea
                ...
                placeholder="例如：Go / 并发编程 / channel；理解阻塞条件并能分析死锁……"
                rows={4}
                className="mt-2 w-full border rounded-lg p-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">当前学习状态（可选）</label>
              <FileUpload
                label="上传历史作答、错题或学习记录（PDF/DOCX）"
                accept=".pdf,.txt,.docx"
              ...
              <textarea
                ...
                placeholder="可填写已学章节、最近错题、已掌握内容和卡点；留空时系统从本轮作答开始诊断。"
                rows={4}
                className="mt-2 w-full border rounded-lg p-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          ...
          {speechAvailable && (
            <label className="mb-4 flex items-center gap-2 text-sm text-gray-700">
              ...
              {ttsAvailable && asrAvailable
                ? 'Student-Coach 朗读任务，并可使用语音作答'
                : ttsAvailable ? 'Student-Coach 将朗读任务' : '可使用语音作答'}
              ）
            </label>
          )}
          <div className="flex justify-end gap-2">
            ...
            <button
              ...
              disabled={!learningGoalText && !learningGoalFile}
      ...
      <div className="border-t px-4 py-3">
        {isTraining && speechAvailable && (
          <div className="mb-2 flex items-center gap-2 text-xs text-gray-600" aria-live="polite">
            ...
            {speechState.kind === 'synthesizing' && <span>正在生成 Student-Coach 语音…</span>}
            {speechState.kind === 'speaking' && <span>Student-Coach 正在提问…</span>}
        ...
        <div className="flex items-end gap-2">
            ...
            <>
              ...
              <div className="relative group">
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-64 px-3 py-2 bg-gray-800 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                  <p className="font-medium mb-1">上传知识点训练题库</p>
                  <p>支持知识理解、分步练习与迁移应用题；系统解析主知识点与教育元数据后入库。</p>
          ...
          <textarea
            ...
            placeholder={isTraining ? '写出你的答案、步骤和依据；也可使用语音作答...' : '咨询知识问题，或点击“开始训练”进入自适应诊断闭环...'}
            rows={1}

...
Student-Coach 是 AI 自适应学习训练与知识掌握诊断系统。本目录为其 React、TypeScript 和 Vite 交互界面。

前端负责登录、学习目标输入、训练对话、语音交互、过程反馈、知识掌握报告和下一轮学习路径展示。完整项目定位见[项目首页](../README.md)。
...
- 上传训练资料；
- 在后端允许时使用问题朗读和语音作答；
- 仅使用 training 语义的前端消息模型，历史协议由后端兼容边界处理。
...
- npm；
- 已启动的 Student-Coach 后端；
- 推荐使用近期版本的 Chrome 或 Edge，以获得完整语音能力。

## 本地启动

1. 先按照[后端说明](../backend/README.md)启动依赖和 Go 服务。
2. 进入 web。
...
4. 查看即时反馈，继续补充或修正思路。
5. 训练结束后查看掌握度评估和学习路径建议。
6. 下一次训练时，系统会结合历史知识掌握画像推荐更合适的内容与难度。
...
- Student-Coach 的问题可以自动朗读，也可以随时跳过；
- 浏览器只在用户主动开始语音作答后申请麦克风权限；
- 实时识别结果会逐步显示；
- ASR final 不会自动触发评分，只进入可编辑输入框，由学生确认后提交；

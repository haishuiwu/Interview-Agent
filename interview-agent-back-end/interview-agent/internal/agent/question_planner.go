...
// Phase 1 prompt：根据目标标准 + 教学档案规划训练方向
const directionPlannerPrompt = `你是一名资深教研员和教师发展培训师。请根据目标教师岗位或考核标准，以及学员教学档案的诊断结果，规划本次教学能力训练的题目方向。
...
1. 【数量硬性要求，必须严格遵守，每个类型都要按难度分档铺满】：
   ...
   - 说明：以上是训练候选题池，训练时会按学员实时表现自适应抽取，不要求全部作答；
     因此每档必须铺满，保证同一难度有足够候选可供持续抽取
2. 题型包含三类，以目标标准为边界、以教学档案和薄弱点为个性化依据：
   - theory：教育理念与师德、课程标准、学科知识、学习评价等基础认知
   - practice：围绕档案中真实的授课、备课、教研、班级管理经历进行复盘和教学设计
   - scenario：试讲片段、课堂突发情况、家校沟通、学情差异等综合情境
3. 每个方向给出一个用于教师题库检索的关键词（search_query），例如"形成性评价"、"课堂追问策略"
4. practice 类必须基于教学档案中的真实信息，context 字段填写相关内容摘要；档案未提供时，改为询问通用教学决策，不得虚构经历
5. 每个方向的 difficulty 必须标注准确，且严格符合第 1 条按难度分档的数量配额（同一 type 下 easy/medium/hard 的方向数量必须达标）
6. 【严禁幻觉】不得杜撰任教经历、教学成果、学生数据或具体课堂事实
7. 题目服务于诊断和训练，不生成录用建议，不考察与教师岗位无关的技术八股
  ...
  "directions": [
    {
      ...
      "search_query": "教师题库检索关键词（如：分层提问）",
      "skills": ["训练和诊断的教学能力点"],
      "context": "教学档案中的相关上下文（practice 类有真实经历时填写）"
    }
  ]
}`

// Phase 2 prompt：根据方向 + 题库匹配结果生成最终题目
const questionAssemblerPrompt = `你是一名资深教研员。请根据训练方向和教师题库匹配结果，生成最终的教学能力训练题目。
...
4. 【LLM 出题基于档案】自行出题时，优先结合学员真实的学段、学科、授课与教研经历
5. 【严禁幻觉】practice 类题目不得杜撰档案中不存在的经历；scenario 类可以给出明确的假设课堂情境
6. 题目 content 必须简洁、可作答，并明确需要学员说明的教学决策
7. 每道题准备 1-2 个苏格拉底式追问，用于引导学员补足依据、步骤或反思
...
func (p *QuestionPlanner) AssembleQuestions(ctx context.Context, jd *imodel.JDAnalysis, match *imodel.ResumeMatchResult, directions *imodel.QuestionDirectionPlan, directionDocs []string) (*imodel.QuestionPlan, error) {
	...
	for i, d := range directions.Directions {
		...
		if d.Context != "" {
			directionsText += fmt.Sprintf("- 教学档案上下文: %s\n", d.Context)
		}
	...
	result.Distribution.Basic = 0
	result.Distribution.Experience = 0
	result.Distribution.Design = 0
	for i := range result.Questions {
		...
		case imodel.QuestionTypeTheory:
			result.Distribution.Basic++
		case imodel.QuestionTypePractice:
			result.Distribution.Experience++
		case imodel.QuestionTypeScenario:
			result.Distribution.Design++
		}
	}

...
var (
	...
	ErrLimitExceeded   = &Error{Code: CodeLimitExceeded, Message: "语音服务繁忙，请继续使用文字面试"}
	ErrEmptyAudio      = &Error{Code: CodeEmptyAudio, Message: "没有可识别的音频"}
	ErrUpstreamTimeout = &Error{
		Code: CodeUpstreamTimeout, Message: "语音服务响应超时，请继续使用文字面试", Retryable: true,
	}
	ErrUpstreamUnavailable = &Error{
		Code: CodeUpstreamUnavailable, Message: "语音服务暂时不可用，请继续使用文字面试", Retryable: true,
	}
	ErrUpstreamProtocol = &Error{
		Code: CodeUpstreamProtocolError, Message: "语音服务响应异常，请继续使用文字面试",
	}
	...
	ErrSpeechBusy = &Error{
		Code: CodeSpeechBusy, Message: "语音服务繁忙，请继续使用文字面试",
	}
	ErrTTSUpstreamError = &Error{
		Code: CodeTTSUpstreamError, Message: "语音合成暂时不可用，请继续使用文字面试", Retryable: true,
	}
	ErrTTSUpstreamTimeout = &Error{
		Code: CodeTTSUpstreamTimeout, Message: "语音合成响应超时，请继续使用文字面试", Retryable: true,
	}
)

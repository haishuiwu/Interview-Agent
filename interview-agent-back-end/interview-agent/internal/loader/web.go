...
// ExtractAbilityStandardFromURL 通过 MCP 抓取网页并提取学习目标与能力标准正文。
func ExtractAbilityStandardFromURL(ctx context.Context, url string, chatModel model.ChatModel) (string, error) {
	...
	prompt := `以下是从网页抓取的内容，其中可能包含学习目标、课程标准、考试大纲或能力要求，也包含无关的导航和广告。
请只提取与学生训练目标相关的正文，包括：学习目标、知识范围、能力要求、实践任务、考核环节与评价重点。
...
` + rawText
	...
	// 检测 LLM 是否真的提取到了有效能力标准（而非返回"无法提取"等错误描述）
	...
	for _, marker := range invalidMarkers {
		if strings.Contains(respLower, marker) {
			return "", fmt.Errorf("loader: 该链接未包含有效的学习目标或能力标准，请直接粘贴文本或上传文件")
		}
	}

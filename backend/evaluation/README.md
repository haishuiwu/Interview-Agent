# Student-Coach Evaluation Benchmark
...
| `tool_calling_test.go` | Tool Selection Accuracy | Student-Coach 实际 Tool 调用链与标注链完全一致 |
...
合计 29 条标注样例。该基线使用确定性的 mock LLM，衡量的是 Agent 编排、选择、知识诊断聚合和学习闭环逻辑，不代表真实模型在开放输入上的泛化准确率。

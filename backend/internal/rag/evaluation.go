...
type QuestionBankEvalResult struct {
	...
	Recall        float64         `json:"recall"`         // 召回率（近似）：能力方向被题库覆盖的比例
	Relevance     float64         `json:"relevance"`      // 语义相关性：检索结果与能力标准的匹配程度
...
type QuestionEval struct {
	...
	Relevant bool   `json:"relevant"` // 是否与能力标准相关
	Reason   string `json:"reason"`   // 判断理由
}

const questionBankEvalPrompt = `你是一个 RAG 检索质量评估专家。请评估以下从题库中检索到的题目对学习目标与能力标准的匹配质量。
...
第一步：逐题判断相关性——每道检索到的题目是否与能力标准相关？
第二步：提取能力标准中的核心能力方向，逐一检查题库是否覆盖
第三步：计算指标
...
- recall（召回率）= 题库覆盖的能力方向数 / 能力标准总方向数。衡量题库对能力标准的覆盖程度。
- relevance（语义相关性，0-1）：整体来看，检索结果与学习目标及能力标准的语义匹配程度。
- question_evals：必须对每道题逐一评估是否与能力标准相关。
- skill_coverage：必须列出能力标准中的每个核心能力方向。`

// EvaluateQuestionBank 评估题库与能力标准的匹配质量（在线题库诊断）。
func (e *Evaluator) EvaluateQuestionBank(ctx context.Context, standardAbilities string, docs []*schema.Document) (*QuestionBankEvalResult, error) {
	var docsText string
	for i, doc := range docs {
		docsText += fmt.Sprintf("[题目%d] %s\n", i+1, doc.Content)
	}

	prompt := fmt.Sprintf(questionBankEvalPrompt, standardAbilities, len(docs), docsText)
	messages := []*schema.Message{
		schema.UserMessage(prompt),
	}

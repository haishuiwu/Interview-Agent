// Package educationtool provides the unified Student-Coach education Tool Registry.
...
// Registry centrally registers Student-Coach data tools in stable order.
...
func NewEducationRegistry(growthService service.StudentGrowthService) (*Registry, error) {
	...
	builders := []struct {
		...
		{ReadTool, func() (einotool.BaseTool, error) {
			return utils.InferTool("get_student_profile", "获取当前学生的基础信息、已有学习记录与已知薄弱知识点；学生身份由系统上下文提供。", r.getStudentProfile)
		}},
		{ReadTool, func() (einotool.BaseTool, error) {
			return utils.InferTool("get_ability_profile", "获取跨轮维护的知识点掌握度、近期作答证据、薄弱点与兼容能力分。个性化学习训练应优先调用。", r.getAbilityProfile)
		}},
		{ReadTool, func() (einotool.BaseTool, error) {
			return utils.InferTool("get_growth_history", "按目标知识点或兼容能力标签查询历史训练记录，用于发现反复错误与掌握度变化。", r.getGrowthHistory)
		}},
		{ReadTool, func() (einotool.BaseTool, error) {
			return utils.InferTool("get_ability_report", "获取最近一次学习诊断报告，包含知识点掌握度、薄弱知识点和逐题证据。", r.getAbilityReport)
		}},
		{ReadTool, func() (einotool.BaseTool, error) {
			return utils.InferTool("search_training_case", "根据知识缺口从现有 RAG 题库检索适配练习。", r.searchTrainingCase)
		}},
		{ReadTool, func() (einotool.BaseTool, error) {
			return utils.InferTool("recommend_training_task", "让 Go 学习服务结合知识掌握度、学习目标和检索练习推荐教学策略 Skill 与训练任务。", r.recommendTrainingTask)
		}},
		{WriteTool, func() (einotool.BaseTool, error) {
			return utils.InferTool("update_ability_profile", "把真实作答评价交给 Go Service 聚合知识掌握度、更新长期学习画像并保存记录；不得传入臆测分数。", r.updateAbilityProfile)
		}},
		{WriteTool, func() (einotool.BaseTool, error) {
...
type growthHistoryRequest struct {
	Ability string `json:"ability" jsonschema:"description=要查询的知识点；字段名为历史兼容标识"`
	Limit   int    `json:"limit,omitempty" jsonschema:"description=最多返回的历史记录数，建议 3 到 5"`
}

type trainingCaseRequest struct {
	AbilityGap string `json:"ability_gap" jsonschema:"description=需要训练的知识缺口；字段名为历史兼容标识"`
...
type recommendTrainingTaskRequest struct {
	...
	Ability      string `json:"ability" jsonschema:"description=本轮重点知识点；字段名为历史兼容标识"`
...
func growthRecordInput(request saveGrowthRecordRequest) service.GrowthRecordInput {
	return service.GrowthRecordInput{
		...
		KnowledgeMastery: request.KnowledgeMastery,

...
// Package model 定义自适应学习训练与知识掌握度诊断的核心数据模型。
package model
...
type AbilityStandard struct {
	...
	Textbook             string            `json:"textbook,omitempty"`    // 教材或课程
	Chapter              string            `json:"chapter,omitempty"`     // 章节或单元
	KnowledgePoints      []string          `json:"knowledge_points"`      // 本轮需诊断与训练的知识点
...
type LearningDiagnosis struct {
	...
	AbilityScores       map[string]float64  `json:"ability_scores"`        // 兼容字段：通用能力当前得分
	KnowledgeMastery    map[string]float64  `json:"knowledge_mastery"`     // 知识点掌握度诊断（0-100）
	WeakKnowledgePoints []string            `json:"weak_knowledge_points"` // 待补强知识点
...
// AbilityGrowthRecord 记录一次训练前后的可追溯学习状态变化；类型名为历史兼容名称。
...
// StudentAbilityProfile 是跨训练持续维护的学习画像；掌握度与兼容能力分数范围均为 0 到 1。
type StudentAbilityProfile struct {
	StudentID         string                `json:"student_id"`
	Summary           string                `json:"summary"`
	KnowledgeMastery  map[string]float64    `json:"knowledge_mastery,omitempty"`
	RecentPerformance []QuestionPerformance `json:"recent_performance,omitempty"`
	AbilityScores     map[string]float64    `json:"ability_scores,omitempty"` // 兼容字段：通用能力分数
	Strengths         []string              `json:"strengths,omitempty"`
	Weaknesses        []string              `json:"weaknesses,omitempty"`
...
// QuestionPerformance 是动态难度和跨轮诊断使用的近期证据。
...
type TrainingState struct {
	...
	CurrentKnowledgePoint string                 `json:"current_knowledge_point,omitempty"`
	CurrentMastery        float64                `json:"current_mastery,omitempty"` // 0-1
	...
	TrainingAttempts      []*TrainingAttempt     `json:"training_attempts,omitempty"`       // 训练事实，评价与学习轨迹的唯一追溯来源
	StudentAbilityProfile *StudentAbilityProfile `json:"student_ability_profile,omitempty"` // 训练中动态更新的知识掌握画像
}
...
type EvaluationReport struct {
	...
	AbilityScores       map[string]float64 `json:"ability_scores"`              // 兼容字段：通用能力维度得分
	KnowledgeMastery    map[string]float64 `json:"knowledge_mastery"`           // 本轮知识点掌握度（0-100）
	WeakKnowledgePoints []string           `json:"weak_knowledge_points"`       // 下一轮优先补强知识点

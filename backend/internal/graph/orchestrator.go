...
// ErrUserQuit 用户主动终止训练。
var ErrUserQuit = errors.New("用户主动终止训练")
...
type Orchestrator struct {
	...
	// 学习状态服务（聚合知识掌握画像并保存学习轨迹；类型名保留历史兼容）
...
func (o *Orchestrator) RunTraining(ctx context.Context, abilityStandardText string, studentProfileText string, userID string, cb *TrainingCallbacks) (*imodel.Session, error) {
	...
	_ = g.AddLambdaNode("learning_goal_analysis", compose.InvokableLambda(func(ctx context.Context, _ string) (string, error) {
		return "", o.nodeAbilityAnalysis(ctx, ic)
	}))
	_ = g.AddLambdaNode("learner_state_analysis", compose.InvokableLambda(func(ctx context.Context, _ string) (string, error) {
		return "", o.nodeStudentProfileAnalysis(ctx, ic)
	}))
	_ = g.AddLambdaNode("training_planning", compose.InvokableLambda(func(ctx context.Context, _ string) (string, error) {
		return "", o.nodeQuestionPlan(ctx, ic)
	}))
	_ = g.AddLambdaNode("adaptive_training", compose.InvokableLambda(func(ctx context.Context, _ string) (string, error) {
		return "", o.nodeTraining(ctx, ic)
	}))
	_ = g.AddLambdaNode("targeted_review", compose.InvokableLambda(func(ctx context.Context, _ string) (string, error) {
		return "", o.nodeWeakReview(ctx, ic)
	}))
	_ = g.AddLambdaNode("session_summary", compose.InvokableLambda(func(ctx context.Context, _ string) (string, error) {
		return "", o.nodeEvaluation(ctx, ic)
	}))
	_ = g.AddLambdaNode("next_training_plan", compose.InvokableLambda(func(ctx context.Context, _ string) (string, error) {
		return "", o.nodeGrowthPlan(ctx, ic)
	}))

	_ = g.AddEdge(compose.START, "learning_goal_analysis")
	_ = g.AddEdge("learning_goal_analysis", "learner_state_analysis")
	_ = g.AddEdge("learner_state_analysis", "training_planning")
	_ = g.AddEdge("training_planning", "adaptive_training")
	// training 之后的条件分支：用户未作答即终止 → END；否则 → weak_review
	_ = g.AddBranch("adaptive_training", compose.NewGraphBranch(
		func(ctx context.Context, _ string) (string, error) { return o.afterTraining(ic), nil },
		map[string]bool{"targeted_review": true, compose.END: true}))
	_ = g.AddEdge("targeted_review", "session_summary")
	_ = g.AddEdge("session_summary", "next_training_plan")
	_ = g.AddEdge("next_training_plan", compose.END)
...
func (o *Orchestrator) nodeAbilityAnalysis(ctx context.Context, ic *trainingCtx) error {
	ic.cb.OnStageChange("learning_goal_analysis", "正在解析学习目标与能力标准...")
	...
	ic.cb.OnStageChange("learning_goal_analysis_done", fmt.Sprintf("能力标准解析完成：%s", standard.LearningGoal))
	return nil
}
...
func (o *Orchestrator) nodeStudentProfileAnalysis(ctx context.Context, ic *trainingCtx) error {
	ic.cb.OnStageChange("learner_state_analysis", "正在建立学生能力训练起点...")
	...
	ic.cb.OnStageChange("learner_state_analysis_done", fmt.Sprintf("学生画像诊断完成，当前能力证据覆盖度：%.0f%%", diagnosis.OverallScore))
	return nil
}
...
func (o *Orchestrator) nodeQuestionPlan(ctx context.Context, ic *trainingCtx) error {
	...
	if profileErr != nil {
		log.Printf("[Profile] 读取长期知识掌握画像失败（继续使用本轮状态）: %v", profileErr)
	} else {
	...
	if len(memoryLines) > 0 {
		...
		ic.cb.OnStageChange("memory_loaded", fmt.Sprintf("已加载 %d 条长期学习状态与历史训练证据，将针对性练习", len(memoryLines)))
	}

	// ===== 阶段 3 Phase 1：规划出题方向 =====
	ic.cb.OnStageChange("training_planning", "正在规划个性化学生能力训练方向...")
	...
	matchedCount := 0
	usedQuestionDocs := make(map[string]bool)
	...
	if hasRAG {
		...
		for i, dir := range dirPlan.Directions {
			...
			queryParts := []string{dir.Subject, dir.Textbook, dir.Chapter, dir.KnowledgePoint, dir.SearchQuery}
			if strings.TrimSpace(dir.SearchQuery) == "" {
				queryParts = append(queryParts, dir.Topic)
			}
			...
			for _, doc := range docs {
				if usedQuestionDocs[doc.ID] || !documentMatchesDirection(doc, dir) {
					continue
				}
			...
			if len(docs) > 0 {
				...
				topDoc := reranked[0]
				usedQuestionDocs[topDoc.ID] = true
				...
				matchedQuestions = append(matchedQuestions, imodel.PlannedQuestion{
					...
					Chapter:        dir.Chapter,
					KnowledgePoint: dir.KnowledgePoint,
	...
	if len(unmatchedDirs) > 0 {
		...
		for i := range llmQuestions {
			if i >= len(unmatchedDirs) {
				break
			}
			if llmQuestions[i].Subject == "" {
				llmQuestions[i].Subject = unmatchedDirs[i].Subject
			}
			if llmQuestions[i].Textbook == "" {
				llmQuestions[i].Textbook = unmatchedDirs[i].Textbook
			}
			if llmQuestions[i].Chapter == "" {
				llmQuestions[i].Chapter = unmatchedDirs[i].Chapter
			}
			if llmQuestions[i].KnowledgePoint == "" {
				llmQuestions[i].KnowledgePoint = unmatchedDirs[i].KnowledgePoint
			}
		}
	}
	...
	ic.cb.OnStageChange("training_planning_done", fmt.Sprintf("训练题池完成，共 %d 道题（基础理解%d/实践应用%d/综合情境%d）",
...
func (o *Orchestrator) nodeTraining(ctx context.Context, ic *trainingCtx) error {
	...
	ic.cb.OnStageChange("adaptive_training", "学生能力训练正式开始！")
	...
	if ic.abilityProfile == nil {
		ic.abilityProfile = &imodel.StudentAbilityProfile{
			...
			KnowledgeMastery: map[string]float64{},
	...
	for {
		...
		state.CurrentStage = q.Type
		state.CurrentKnowledgePoint = q.KnowledgePoint
		state.CurrentMastery = 0.5
		if mastery, ok := state.StudentAbilityProfile.KnowledgeMastery[q.KnowledgePoint]; ok {
			state.CurrentMastery = mastery
		}
		q.Difficulty = string(difficulty)
		...
		if shouldFollowUp && attempt.FollowUpCount < maxFollowUpsPerQuestion {
			...
			if fErr == nil && followUpText != "" && !strings.EqualFold(followUpText, questionText) {
				...
				if faErr == nil {
					...
					if fsErr == nil {
						...
						// 追问是带支架证据：取主答与折损后追问的较高值，避免既忽略补救也高估提示后表现。
						followUpEvidence := followUpScore.Score * 0.85
						if followUpEvidence > qa.Score {
							qa.Score = followUpEvidence
							qa.Feedback = score.Feedback + "；追问后补充：" + followUpScore.Feedback
						}
						attempt.HintUsed = true
						attempt.FollowUpCount = 1
						attempt.MasteryEvidenceScore = qa.Score
					}
		...
		// 主答与最多一次追问合并后，只更新一次掌握度，避免把同一题重复计权。
		...
		if profileErr != nil {
			log.Printf("[Profile] 学习画像更新失败（不影响主流程）: %v", profileErr)
		} else {
			state.StudentAbilityProfile = updatedProfile
			ic.abilityProfile = updatedProfile
		}
		state.QAHistory = append(state.QAHistory, qa)
		state.RecentScores = append(state.RecentScores, qa.Score)
		if len(state.RecentScores) > 5 {
			state.RecentScores = state.RecentScores[len(state.RecentScores)-5:]
		}

		// 动态难度调节使用合并后的有效证据分。
		sched.setEvidenceContext(state.CurrentMastery, state.CurrentStage)
		sched.record(qa.Score)
		state.ConsecutiveRight = sched.consecRight
		state.ConsecutiveWrong = sched.consecWrong

		// 更新薄弱知识点（长期记忆 → Redis + MySQL）；无知识点元数据时兼容旧技能字段。
		weakTargets := append([]string(nil), q.Skills...)
		if strings.TrimSpace(q.KnowledgePoint) != "" {
			weakTargets = []string{q.KnowledgePoint}
		}
		for _, knowledgePoint := range weakTargets {
			_ = o.longTermMem.UpdateWeakPoints(ctx, userID, knowledgePoint, qa.Score)
		}
	}
...
func (o *Orchestrator) nodeWeakReview(ctx context.Context, ic *trainingCtx) error {
	...
	ic.cb.OnStageChange("targeted_review", fmt.Sprintf("正在对 %d 道低分题目进行巩固...", len(weakQAs)))
...
func (o *Orchestrator) nodeEvaluation(ctx context.Context, ic *trainingCtx) error {
	...
	if ic.userTerminated {
		ic.cb.OnStageChange("session_summary", fmt.Sprintf("训练提前终止，正在基于已完成的 %d 道题生成知识掌握诊断...", len(state.QAHistory)))
	} else {
		ic.cb.OnStageChange("session_summary", "正在生成学生能力训练评估报告...")
...
// nodeGrowthPlan 阶段 6：生成下一轮学习路径 + 持久化训练记录。
func (o *Orchestrator) nodeGrowthPlan(ctx context.Context, ic *trainingCtx) error {
	ic.cb.OnStageChange("next_training_plan", "正在生成下一轮学习路径...")
	...
	// 知识掌握画像与兼容 GrowthRecord 已在 evaluation 节点由 StudentGrowthService 保存；
	// 此处只补齐现有记录中的学习路径 JSON。
	if o.mysqlStore != nil {
...
func newTrainingAttempt(studentID string, planned imodel.PlannedQuestion, question, reference, attemptType, parentAttemptID string) *imodel.TrainingAttempt {
	...
	return &imodel.TrainingAttempt{
		...
		KnowledgePoints: knowledgePointsForPlannedQuestion(planned),
...
func knowledgePointsForPlannedQuestion(question imodel.PlannedQuestion) []string {
...
func documentMatchesDirection(doc *schema.Document, direction imodel.QuestionDirection) bool {
	if doc == nil {
		return false
	}
	return metadataMatches(doc.MetaData, "subject", direction.Subject) &&
		metadataMatches(doc.MetaData, "textbook", direction.Textbook) &&
		metadataMatches(doc.MetaData, "chapter", direction.Chapter) &&
		metadataMatches(doc.MetaData, "knowledge_point", direction.KnowledgePoint)
}

func metadataMatches(metadata map[string]any, key, expected string) bool {
	...
	if !exists {
		return true // 兼容尚未补齐教育元数据的历史题库
	}
	actual := strings.TrimSpace(fmt.Sprint(actualValue))
	if actual == "" {
		return true
	}
	return strings.EqualFold(actual, expected) ||
		strings.Contains(strings.ToLower(actual), strings.ToLower(expected)) ||

...
const questionParserPrompt = `你是一名学习训练题库解析专家。请从文本中提取可诊断具体知识掌握度的题目，并转换为结构化 JSON。
...
[
  {
    "subject": "学科或学习领域",
    "textbook": "教材或课程（无法判断时为空）",
    "chapter": "章节或单元（无法判断时为空）",
    "knowledge_point": "该题唯一主知识点",
    "content": "完整的知识理解、练习或迁移应用问题",
...
3. knowledge_point 必须是非空、可检索、可评价的具体概念或方法名；skills 为兼容字段，填写本题需要的应用或思考能力。
...
func validateQuestion(q *ParsedQuestion) error {
	...
	q.Textbook = strings.TrimSpace(q.Textbook)
	q.Chapter = strings.TrimSpace(q.Chapter)
	q.KnowledgePoint = strings.TrimSpace(q.KnowledgePoint)

	if q.KnowledgePoint == "" {
		return fmt.Errorf("knowledge_point 为空")
	}
	if len(q.Content) < 10 {

...
// growthPlannerInstruction ReAct 系统指令：按知识掌握诊断生成下一轮学习路径。
const growthPlannerInstruction = `你是 Student-Coach 的学习路径规划器，要根据知识掌握诊断和真实作答证据制定下一轮学习计划。

资源优先级依次为：权威课程或考试标准、教材与参考书、优质课程、经典书籍和可执行的刻意练习。
只有当待补强知识点明确涉及编程、数据分析、数字工具或技术实践时，才可以调用 search_github_repos 搜索真实开源工具；
不得为了展示工具调用而推荐与训练目标无关的技术项目。工具不可用或没有合适结果时，不编造名称和链接。

规划原则：优先处理低掌握度和高频错误知识点；每个学习项包含可观察目标、针对性练习、错因复盘与复测方式；时间估算合理；不对学生作人格或职业定性。

最终只输出纯 JSON（不要输出工具调用过程、思考说明或多余文字），格式：
{
  "weak_areas": [
    {"topic": "薄弱知识点", "score": 50.0, "priority": "high/medium/low"}
  ],
  "study_plan": [
    {"topic": "知识点或训练主题", "objective": "可观察的掌握目标", "actions": ["概念回顾", "针对性练习", "错因复盘与复测"], "time_estimate": "预估时间"}
  ],
  "resources": [
...
// GrowthPlanner 学习路径规划 Agent —— 保留原有 Eino ReAct 与 MCP 工具调用机制。
type GrowthPlanner struct {
	chatModel      model.ChatModel
	githubSearcher *mcp.GitHubSearcher // 可选，nil 时退化为纯 LLM 生成
}

// NewGrowthPlanner 创建学习路径规划 Agent。

...
const abilityEvaluatorPrompt = `你是 Student-Coach 的知识掌握评估器。请根据学生的完整训练表现，生成形成性的掌握度诊断报告。

报告只用于训练反馈与下一轮学习路径规划，不对学生作人格或职业定性。评价必须能从实际作答中找到证据；样本不足时应在 summary 中说明。
...
func (e *AbilityEvaluator) Evaluate(ctx context.Context, state *imodel.TrainingState, standard *imodel.AbilityStandard, profile *imodel.StudentProfile, userTerminated bool) (*imodel.EvaluationReport, error) {
	...
	result.KnowledgeMastery = calculateKnowledgeMastery(state)
	result.WeakKnowledgePoints = weakKnowledgePoints(result.KnowledgeMastery)
...
func calculateFinalAbilityScores(state *imodel.TrainingState) (map[string]float64, float64) {
	...
	if len(attempts) > 0 {
		...
		for _, attempt := range attempts {
			score := math.Max(0, math.Min(100, attempt.EvidenceScore()))
			overallTotal += score
...
func calculateKnowledgeMastery(state *imodel.TrainingState) map[string]float64 {
	totals := make(map[string]float64)
	counts := make(map[string]int)
	for _, attempt := range evaluatedTrainingAttempts(state, false) {
		for _, knowledgePoint := range attempt.KnowledgePoints {
			knowledgePoint = strings.TrimSpace(knowledgePoint)
			if knowledgePoint == "" {
				continue
			}
			totals[knowledgePoint] += attempt.EvidenceScore()
			counts[knowledgePoint]++
		}
	}
	result := make(map[string]float64, len(totals))
	for knowledgePoint, total := range totals {
		result[knowledgePoint] = math.Round(total/float64(counts[knowledgePoint])*100) / 100
	}
	return result
}

func weakKnowledgePoints(mastery map[string]float64) []string {
	result := make([]string, 0)
	for knowledgePoint, score := range mastery {
		if score < 60 {
			result = append(result, knowledgePoint)
		}
	}
	sort.Strings(result)
	return result
...
func FormatReport(report *imodel.EvaluationReport) string {
	...
	if len(report.KnowledgeMastery) > 0 {
		md += "## 知识点掌握度\n\n"
		md += "| 知识点 | 掌握度 |\n|--------|--------|\n"
		keys := make([]string, 0, len(report.KnowledgeMastery))
		for knowledgePoint := range report.KnowledgeMastery {
			keys = append(keys, knowledgePoint)
		}
		sort.Strings(keys)
		for _, knowledgePoint := range keys {
			md += fmt.Sprintf("| %s | %.1f |\n", knowledgePoint, report.KnowledgeMastery[knowledgePoint])
		}
		md += "\n"
	}

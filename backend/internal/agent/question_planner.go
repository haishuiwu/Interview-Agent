...
3. 每个方向必须绑定一个明确知识点，并给出学科、教材/课程、章节以及用于题库检索的关键词（search_query）；检索词应包含知识点与典型错误或应用条件
...
7. 题目服务于知识掌握诊断和针对性训练，不对学生作人格或职业定性
...
func (p *QuestionPlanner) PlanDirections(ctx context.Context, standard *imodel.AbilityStandard, diagnosis *imodel.LearningDiagnosis, weakPoints string) (*imodel.QuestionDirectionPlan, error) {
	...
	for i := range result.Directions {
		...
		direction.Type = imodel.NormalizeQuestionType(direction.Type)
		if direction.Subject == "" {
			direction.Subject = standard.Subject
		}
		if direction.Textbook == "" {
			direction.Textbook = standard.Textbook
		}
		if direction.Chapter == "" {
			direction.Chapter = standard.Chapter
		}
		if direction.KnowledgePoint == "" && len(standard.KnowledgePoints) > 0 {
			direction.KnowledgePoint = standard.KnowledgePoints[i%len(standard.KnowledgePoints)]
		}
	}
...
func (p *QuestionPlanner) AssembleQuestions(ctx context.Context, standard *imodel.AbilityStandard, diagnosis *imodel.LearningDiagnosis, directions *imodel.QuestionDirectionPlan, directionDocs []string) (*imodel.QuestionPlan, error) {
	...
	for i, d := range directions.Directions {
		...
		directionsText += fmt.Sprintf("- 学科: %s, 教材/课程: %s, 章节: %s, 主知识点: %s\n", d.Subject, d.Textbook, d.Chapter, d.KnowledgePoint)
		directionsText += fmt.Sprintf("- 类型: %s, 难度: %s, 兼容能力标签: %v\n", d.Type, d.Difficulty, d.Skills)
		if d.Context != "" {
	...
	for i := range result.Questions {
		...
		question.Type = imodel.NormalizeQuestionType(question.Type)
		if i < len(directions.Directions) {
			direction := directions.Directions[i]
			if question.Subject == "" {
				question.Subject = direction.Subject
			}
			if question.Textbook == "" {
				question.Textbook = direction.Textbook
			}
			if question.Chapter == "" {
				question.Chapter = direction.Chapter
			}
			if question.KnowledgePoint == "" {
				question.KnowledgePoint = direction.KnowledgePoint
			}
		}

// Package service 包含由 Go 控制的学习状态、掌握度和训练轨迹服务。
package service
...
// StudentProfileSnapshot 是 Tool 可读取的学习状态视图。
...
type AbilityReportSnapshot struct {
	...
	KnowledgeMastery map[string]float64 `json:"knowledge_mastery,omitempty"`
...
// AbilityProfileUpdateResult 返回更新后的知识掌握画像和对应学习轨迹记录。
...
// StudentGrowthDataService 使用现有 Memory、评价 JSON 与 RAG 提供学习状态数据；类型名为历史兼容名称。
...
// NewStudentGrowthDataService 创建学习状态数据服务，不创建或修改数据库结构。
...
// GetAbilityProfile 读取长期知识掌握画像；首次训练时返回空画像而不是虚构分数。
func (s *StudentGrowthDataService) GetAbilityProfile(ctx context.Context, studentID string) (*imodel.StudentAbilityProfile, error) {
	if s.store == nil {
		return &imodel.StudentAbilityProfile{StudentID: studentID, KnowledgeMastery: map[string]float64{}, AbilityScores: map[string]float64{}}, nil
	}
	...
	if profile == nil {
		profile = &imodel.StudentAbilityProfile{StudentID: studentID, KnowledgeMastery: map[string]float64{}, AbilityScores: map[string]float64{}}
	}
	...
	if profile.KnowledgeMastery == nil {
		profile.KnowledgeMastery = map[string]float64{}
	}
	return profile, nil
}

// UpdateAbilityProfile 由 Go 聚合本轮证据与历史状态，并在画像更新后保存学习轨迹。
...
func (s *StudentGrowthDataService) UpdateAbilityProfile(ctx context.Context, studentID string, input GrowthRecordInput) (*AbilityProfileUpdateResult, error) {
	...
	for knowledgePoint, rawScore := range input.KnowledgeMastery {
		knowledgePoint = strings.TrimSpace(knowledgePoint)
		if knowledgePoint == "" {
			continue
		}
		evidenceScore := normalizeAbilityScore(rawScore)
		if previous, exists := profile.KnowledgeMastery[knowledgePoint]; exists {
			evidenceScore = (previous + evidenceScore) / 2
		}
		profile.KnowledgeMastery[knowledgePoint] = roundAbilityScore(evidenceScore)
	}
...
func (s *StudentGrowthDataService) RecommendTrainingTask(ctx context.Context, studentID, learningGoal, ability, caseContent string) (*TrainingTaskRecommendation, error) {
	...
	if weakestScore > 0 {
		reason = fmt.Sprintf("长期学习状态的兼容评价维度中 %s 得分最低（%.2f），因此优先选择 %s。", selectedAbility, weakestScore, skillName)
	}
	return &TrainingTaskRecommendation{
		SkillName:    skillName,
		Ability:      selectedAbility,
		Objective:    "通过作答证据定位当前学习问题，并形成一个可执行的训练动作。",
		TrainingTask: task,
		Reason:       reason,
	}, nil
}
...
func RecommendStartingDifficulty(profile *imodel.StudentAbilityProfile) imodel.DifficultyLevel {
	...
	_, score := weakestKnowledgePoint(profile)
	if len(profile.KnowledgeMastery) == 0 {
		ability, abilityScore := weakestProfileAbility(profile)
		if ability == "" {
			return imodel.DifficultyMedium
		}
		score = abilityScore
...
func weakestKnowledgePoint(profile *imodel.StudentAbilityProfile) (string, float64) {
	if profile == nil || len(profile.KnowledgeMastery) == 0 {
		return "", 0
	}
	selected := ""
	selectedScore := 0.0
	for knowledgePoint, score := range profile.KnowledgeMastery {
